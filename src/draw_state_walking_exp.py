...
walk_exp_month = np.floor(
    np.array(list(infant_info['walking_exp'].values()))/30)
...
for task in tasks:
    ...
    each_cond_time_novice_exp[task] = {"novice": {}, "exp": {}}
    ...
    for state in state_name_dict.values():
        ...
        each_cond_time_novice_exp[task]["novice"][state] = []
        each_cond_time_novice_exp[task]["exp"][state] = []
        for i in range(1, 11):
...
for task in tasks:
    for subj in subj_list:
        ...
        walk_exp_month_2_mon = np.digitize(
            walking_exp, bins=bins_2, right=False)
        ...
        for idx, state in enumerate(pred_unique):
            ...
            each_cond_time[task][str(state)][walk_exp_month].append(
                cnt[idx]/cnt.sum())
            each_cond_time_2_mon[task][state][walk_exp_month_2_mon].append(
                cnt[idx]/cnt.sum())
            each_cond_time_3_mon[task][state][walking_exp_3_mon].append(
                cnt[idx]/cnt.sum())

            each_cond_cnt[task][state].append(walking_exp)
            all_task_time[str(state)][walk_exp_month].append(
                cnt[idx]/cnt.sum())
            all_task_time_2_mon[str(state)][walk_exp_month_2_mon].append(
                cnt[idx]/cnt.sum())
            all_task_time_3_mon[str(state)][walking_exp_3_mon].append(
                cnt[idx]/cnt.sum())

            all_task_dict[str(state)].append(walking_exp)

            if infant_info['walking_exp'][subj] < 90:
                each_cond_time_novice_exp[task]["novice"][state].append(
                    cnt[idx]/cnt.sum())
            else:
...
fig, axs = plt.subplots(nrows=1, ncols=len(
    state_name_dict.values()), figsize=(20, 10), sharey=True)
plt.suptitle(
    "Avg. time spent in each state for each walking exp. group", fontsize=26)
...
for idx, state in enumerate(state_name_dict.values()):
    ...
    for month_pos, val in all_task_time[state].items():
        if len(val) == 0:
            ...
            axs[idx].bar(month_pos, np.mean(val),
                         color=state_color_dict[state])
            axs[idx].errorbar(month_pos, np.mean(val), yerr=(
                [0], [np.std(val)]), barsabove=True, color='dimgray')
...
axs[0].set_yticklabels([str(np.round(i, 1))
                        for i in np.arange(0, 0.9, 0.1)], fontsize=24)
...
fig, axs = plt.subplots(nrows=1, ncols=len(
    state_name_dict.values()), figsize=(20, 10), sharey=True)
plt.suptitle(
    "Distribution of walking experience (days) in each state", fontsize=18)
...
fig, axs = plt.subplots(nrows=4, ncols=len(
    state_name_dict.values()), figsize=(20, 15), sharex=True, sharey=True)
plt.suptitle(
    "Pct. of each session in each state for each walking exp. group", fontsize=32)
    ...
    for col_idx, state in enumerate(state_name_dict.values()):
        for month_pos, all_pct in each_cond_time_2_mon[task][str(state)].items():
            if len(all_pct) == 0:
                ...
                axs[row_idx, col_idx].bar(month_pos, np.mean(
                    all_pct), color=state_color_dict[state])
                axs[row_idx, col_idx].errorbar(month_pos, np.mean(all_pct), yerr=(
                    [0], [np.std(all_pct)]), barsabove=True, color='dimgray')
            axs[row_idx, col_idx].grid(False)
        axs[row_idx, col_idx].set_xticks(
            list(each_cond_time_2_mon[task][str(state)].keys()))
...
fig, axs = plt.subplots(nrows=4, ncols=6, figsize=(
    20, 15), sharex=True, sharey=True)
plt.suptitle(
    ...
    for state_pos, state in enumerate(state_name_dict.values()):
        for col_idx, (month_pos, all_pct) in enumerate(each_cond_time_2_mon[task][state].items()):
            if len(all_pct) == 0:
                ...
                axs[row_idx, col_idx].bar(state_pos, np.mean(
                    all_pct), color=state_color_dict_shades[state])
                axs[row_idx, col_idx].errorbar(state_pos, np.mean(all_pct), yerr=(
                    [0], [np.std(all_pct)]), barsabove=True, color='dimgray')
            axs[row_idx, col_idx].grid(False)
            axs[row_idx, col_idx].set_xticks(
                list(range(len(state_name_dict.values()))))
            axs[row_idx, col_idx].set_xticklabels(
                state_name_dict.values(), fontsize=26)

            if row_idx == 0:
                axs[row_idx, col_idx].set_xlabel(
                    str(month_pos*2) + " month(s)", fontsize=26)
                axs[row_idx, col_idx].xaxis.set_label_position('top')
            if row_idx == len(tasks):
                axs[row_idx, 0].tick_params("y", left=False, labelleft=False)
                axs[row_idx, -1].tick_params("y", right=True, labelright=True)
    ...
    for state_pos, state in enumerate(state_name_dict.values()):
        for col_idx, (month_pos, all_pct) in enumerate(each_cond_time_3_mon[task][state].items()):
            if len(all_pct) == 0:
                ...
                axs[row_idx, col_idx].bar(state_pos, np.mean(
                    all_pct), color=state_color_dict_shades[state], alpha=0.8)
                axs[row_idx, col_idx].errorbar(state_pos, np.mean(all_pct), yerr=(
                    [0], [np.std(all_pct)]), barsabove=True, color='dimgray')
            axs[row_idx, col_idx].grid(False)
            axs[row_idx, col_idx].set_xticks(
...
# %%
mom_no_mom_3_mon = {'Without_CG': {}, 'With_CG': {}, }
for state_pos, state in enumerate(state_name_dict.values()):
    for task in tasks:
        if task in ["NMS", 'NMM']:
            mom_indicator = 'Without_CG'
        else:
            mom_indicator = 'With_CG'

        if not str(state) in mom_no_mom_3_mon[mom_indicator]:
            mom_no_mom_3_mon[mom_indicator][str(state)] = {}
            # for group in each_cond_time_3_mon[task][state].keys():
            mom_no_mom_3_mon[mom_indicator][str(
                state)] = each_cond_time_3_mon[task][str(state)]
...
plt.suptitle(
    "Pct. of each session in each state for each walking exp. group", fontsize=28)
xlabels_list = ['0-3 month(s)', '3-6 months', '6+ months']
for row_idx, mom_cond in enumerate(list(mom_no_mom_3_mon.keys())):
    for state_pos, state in enumerate(state_name_dict.values()):
        for col_idx, (month_pos, all_pct) in enumerate(mom_no_mom_3_mon[mom_cond][state].items()):
            # print(month_pos, all_pct)
            if len(all_pct) == 0:
                axs[row_idx, col_idx].bar(state_pos, 0)
            else:
                axs[row_idx, col_idx].bar(state_pos, np.mean(
            ...
            if col_idx == 0:
                axs[row_idx, col_idx].set_ylabel(mom_cond, fontsize=26)
                axs[row_idx, col_idx].set_yticks([0, 0.2, .4, .6, .8])
                axs[row_idx, col_idx].set_yticklabels(
                    [0, 20, 40, 60, 80], fontsize=26)
fig.text(0.5, -0.01, 'States', ha='center', fontsize=26)
...
# %%
toys_3_mon = {'Gross-motor': {}, 'Fine-motor': {}, }
for state_pos, state in enumerate(state_name_dict.values()):
    for task in tasks:
        if task in ["MPM", 'NMM']:
            toy_indicator = 'Gross-motor'
        else:
            toy_indicator = 'Fine-motor'

        if not str(state) in toys_3_mon[toy_indicator]:
            toys_3_mon[toy_indicator][str(state)] = {}
            # for group in each_cond_time_3_mon[task][state].keys():
            toys_3_mon[toy_indicator][str(
                state)] = each_cond_time_3_mon[task][str(state)]
...
plt.suptitle(
    "Pct. of each session in each state for each walking exp. group", fontsize=28)
xlabels_list = ['0-3 month(s)', '3-6 months', '6+ months']
for row_idx, toy_cond in enumerate(list(toys_3_mon.keys())):
    for state_pos, state in enumerate(state_name_dict.values()):
        for col_idx, (month_pos, all_pct) in enumerate(toys_3_mon[toy_cond][state].items()):
            # print(month_pos, all_pct)
            if len(all_pct) == 0:
                axs[row_idx, col_idx].bar(state_pos, 0)
            else:
            ...
            if col_idx == 0:
                axs[row_idx, col_idx].set_ylabel(toy_cond, fontsize=26)
                axs[row_idx, col_idx].set_yticks([0, 0.2, .4, .6, .8])
                axs[row_idx, col_idx].set_yticklabels(
                    [0, 20, 40, 60, 80], fontsize=26)
fig.text(0.5, -0.01, 'States', ha='center', fontsize=26)
...
fig, axs = plt.subplots(nrows=1, ncols=len(
    state_name_dict.values()), figsize=(20, 10), sharey=True, sharex=True)
plt.suptitle(
    "Distribution of walking experience (months) in each state", fontsize=32)
...
for idx, state in enumerate(state_name_dict.values()):
    ...
    axs[idx].set_yticklabels([str(np.round(i, 2))
                              for i in np.arange(0, 0.31, 0.05)], fontsize=28)
...
for row_idx, task in enumerate(each_cond_cnt.keys()):
    for col_idx, state in enumerate(each_cond_cnt[task].keys()):
        ...
        cnt = cnt/cnt.sum()
        axs[row_idx, col_idx].bar(
            unique_months, cnt, color=state_color_dict[state])
        axs[row_idx, col_idx].grid(False)
        axs[row_idx, col_idx].set_xticks(list(range(1, 11)))
        axs[row_idx, col_idx].set_xticklabels(
            [str(i) for i in range(1, 11)], fontsize=28)
        if row_idx == len(tasks):
            axs[row_idx, 0].tick_params("y", left=False, labelleft=False)
            axs[row_idx, -1].tick_params("y", right=True, labelright=True)
        ...
        if col_idx == 0:
            ...
            axs[row_idx, col_idx].set_yticklabels(
                [0, 0.2, .4, .6, .8, 1], fontsize=28)
        if row_idx == 0:
            axs[row_idx, col_idx].set_xlabel("state " + state, fontsize=28)
            axs[row_idx, col_idx].xaxis.set_label_position('top')

fig.text(0.5, -0.01, 'Walking exp. (in months)', ha='center', fontsize=28)
fig.text(-0.01, 0.5, 'Distribution of walking exp.',
         va='center', rotation='vertical', fontsize=28)
