                    ...
                    for subj in subj_list:
                        for task in tasks:
                            ...
                            for shift_time in np.arange(0, interval_length, shift):
                                ...
                                path = Path('./figures/hmm/20210907/'+feature_set+'/no_ops_threshold_'+str(no_ops_time)+'/window_size_'+str(interval_length)+'/'+str(n_states)+'_states/all_sequences/'+task+'/')
                            ...
                            if len(time_list) != len(prob_list):
                                print(subj, task, shift_time)
                            fig_name = './figures/hmm/20210907/'+feature_set+'/no_ops_threshold_'+str(no_ops_time)+'/window_size_'+str(interval_length)+'/'+str(n_states)+'_states/all_sequences/'+task+'/'+str(subj)+".png"
                            ...
                            list_of_time_list.append(time_list)
                            prob_list = merged_proba_dict_all[task][subj]
                            list_of_prob_list.append(prob_list)
                            ...
                            draw_timeline_with_prob_to_compare(title = str(subj) + " window size: " + str(interval_length) + " no ops threshold "+ str(no_ops_time) + " shift time: " + str(shift*60)+"s", \
                                                        ...
                                                        state_color_dict= state_color_dict_shades, list_of_prob_list = list_of_prob_list, shift = shift)
