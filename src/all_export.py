...
for feature_set in ['n_new_toy_ratio']:#, 'fav_toy_till_now', 'n_new_toy_ratio_and_fav_toy_till_now', 'new_toy_play_time_ratio', 'new_toy_play_time_ratio']:
    for no_ops_time in [5, 7, 10]:
        ...
        for interval_length in [1.5, 1, 2]:
            ...
            with open("./data/interim/20210907_"+str(no_ops_time)+"_no_ops_threshold_feature_engineering_"+str(interval_length)+"_min.pickle", 'rb') as f:
            ...
            for n_states in range(4, 7):
                ...
                Path('./data/result/prediction/20210907/'+feature_set+'/no_ops_threshold_'+str(no_ops_time)+'/window_size_'+str(interval_length)+'/state_'+str(n_states)).mkdir(parents = True, exist_ok = True)
