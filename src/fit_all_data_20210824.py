        ...
        for interval_length in [1.5, 2, 1]:
            ...
            for n_states in range(4, 7):
                ...
                for task in tasks:
                    ...
                    df = pd.DataFrame(data = np.vstack((np.round(normalized_trans_matrix,3), first_state)))
                    df = df.rename(columns=state_name_dict) 
                    index = state_name_dict
                    index[n_states] = 'init_prob'
                    df = df.rename(index = index) 
                    file_name = 'transmat_'+str(task)+'_state_'+str(n_states)+'.csv'
                    save_csv(df, file_path, file_name)
                ...
                for task in tasks:
                    ...
                    merged_pred_dict_all_task_specific, merged_proba_dict_all_task_specific, time_subj_dict_all_task_specific, all_proba = merge_segment_with_state_calculation_all(subj_list, shift_time_list, merged_df_dict, time_arr_shift_dict, pred_subj_dict, prob_subj_dict, window_size = interval_length, n_states = n_states, shift_interval = 60000*shift)
